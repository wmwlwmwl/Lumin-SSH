...
Lumin 是一款面向开发者和运维人员的桌面 SSH 客户端。基于 Go 原生并发 + WebSocket + xterm.js 构建，提供低延迟终端体验。内置系统资源探针、远程文件管理器、命令历史与智能补全、可选加密云同步、AI 对话与 MCP 集成等功能，无需在服务器上安装任何 Agent。
...
- **导入/导出** — 主机列表工具栏的数据管理入口支持导出全部节点（含凭据）为 **明文 JSON** 或 **密文 .lumin2**；密文导出可复用恢复密码或用自定义密码加密；导入时智能识别明文 JSON、`.lumin2` 与旧 `.enc`，密文自动尝试恢复密码/旧版云同步密钥，失败则提示输入密码；提供导入模板下载，方便批量录入与跨机器迁移
...
- **MCP 客户端管理** — 支持添加外部 MCP 服务器，查看工具与资源，并进行启停、重载、重启、删除和超时配置
...
- **可选加密备份** — 设置恢复密码后生成 `.lumin2` 密文备份；未设置时使用便于迁移的 `.json` 快照；旧 `.enc` 仅保留导入/恢复兼容
- **多端一键恢复** — 在新设备上配置同步后端后即可恢复服务器、凭据、快捷指令、AI 配置、代理节点等数据
- **智能合并** — 根据各项数据的更新时间自动合并，并传播删除状态，降低多设备覆盖风险
...
- **字体管理器** — 支持导入、搜索和管理 `.ttf` / `.otf` / `.ttc` / `.woff` / `.woff2`，可分别应用于界面、终端和 AI 面板
...
| **外观** | 字体管理器、终端字体大小、终端输入回显、终端颜色主题、界面主题、强调色、终端壁纸 |
| **快捷键** | 所有终端操作快捷键的自由绑定 |
| **同步与云** | WebDAV / R2 / FTP / SFTP、恢复密码、备份保留与自动同步策略 |
| **AI** | 供应商、模型、提示词缓存、工具审批、MCP 服务与外部 MCP 客户端 |
| **关于** | 版本信息、检查更新、社区链接 |
...
设置 → 同步与云 → 配置 WebDAV / R2 / FTP / SFTP 任一后端。需要密文备份时请先设置恢复密码，Lumin 会生成 `.lumin2` 快照；未设置恢复密码时同步 `.json` 快照。旧 `.enc` 仅用于导入/恢复兼容。在新设备上配置同一后端即可恢复。
